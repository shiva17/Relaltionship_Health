...
"2) We don't have time at home as partners. Select 'Never' if you have time and  'Always' if you do not have time.",
...
def Contact():
    ...
    st.text("Email: kolhe.shivam17@gmail.com")
    st.text("Linkedin: https://in.linkedin.com/in/shivamkolhe8")
...
def Feedback():
    ...
    ratings = st.slider("Ratings", 1, 5)
    print("Ratings", ratings)

    if(st.button('Submit Feedback')):
        st.success('Feedback Submitted')
        #df_feedback = df_feedback.append({"ID":name.title(), "Feedback":feedback.title(), "Rating":ratings}, ignore_index=True)
...
# Selection box
page = st.sidebar.selectbox("Select", ['Relationship Health Analyzer', 'Contact Info', 'Feedback'])

if(page == "Relationship Health Analyzer"):
    RHA()
